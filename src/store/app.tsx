...
import type { Accent, Attributes, Board, Camera, ID, Point, ThoughtType } from '../model/types';
import { boardBounds, boundsOfNodes, buildIndex, descendants, linkedTo, type GraphIndex } from '../model/graph';
import * as C from '../commands/board';
import { loadBoard, loadCamera, makeBoardSaver, saveCamera } from '../persist/db';
import { centerOn, fitRect, interpolateCamera, MAX_ZOOM, MIN_ZOOM } from '../canvas/camera';
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [viewport, setViewportState] = useState({ w: 1440, h: 900 });
  const viewportRef = useRef(viewport);
  const animation = useRef<number>();

  const cameraSaveTimer = useRef<number>();
  const setCamera = useCallback((c: Camera | ((prev: Camera) => Camera)) => {
    ...
    window.clearTimeout(cameraSaveTimer.current);
    cameraSaveTimer.current = window.setTimeout(() => saveCamera(cameraRef.current), 600);
  }, []);
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const [stored, storedCamera] = await Promise.all([loadBoard(), loadCamera()]);
      if (cancelled) return;
  ...
  useEffect(() => {
    const onLeave = () => saver.flushNow(boardRef.current);
