import type { Accent, Board, ID, Rect, Thought } from './types';
import { measure, padding } from '../canvas/typography';
import { fieldPad, ringPigment } from '../canvas/lava';
import { BLOB_SPAN } from '../canvas/blob';
...
/**
 * What colour a thought is drawn in.
 *
 * A map is not confetti. Colour here means *which branch of my thinking is
 * this*, so a thought takes the pigment of the nearest ancestor anyone has
 * actually chosen one for, and if nobody ever has, the whole branch takes one
 * off the ring together — seeded by the branch's own head, so Health is teal
 * all the way down and Growth is blue all the way down.
 *
 * The root belongs to no branch and is seeded by itself.
 */
export function pigmentOf(board: Board, id: ID): Accent {
  const chain: Thought[] = [];
  let cur: Thought | undefined = board.nodes[id];
  while (cur) {
    if (cur.accent !== 'none') return cur.accent;
    chain.push(cur);
    cur = cur.parentId ? board.nodes[cur.parentId] : undefined;
  }
  if (!chain.length) return 'none';
  // chain runs child -> root; the branch head is the one just below the root
  const head = chain[Math.max(chain.length - 2, 0)];
  return ringPigment(head.id);
}

/**
 * The node's box in world space: as much room as its rim asks for.
 *
 * This is what a sibling is spaced against, what an export crops to, and what
 * culling tests — so it has to be the bubble the reader sees, not the smaller
 * box the words sit in. Measuring the text alone would lay neighbours out to
 * touch, and then draw two visible outlines through each other.
 */
export function boxOf(node: Thought): Rect {
  ...
  const w = (m.w / 2 + fp.x) * 2 * BLOB_SPAN;
  const h = (m.h / 2 + fp.y) * 2 * BLOB_SPAN;
