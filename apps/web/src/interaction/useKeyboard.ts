...
import { childrenOf, descendants, type ID, siblingsOf, subtree, worldToScreen } from '@field/core';
...
export function useKeyboard() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      // An account sheet owns the keyboard while it is up — including its own
      // Escape. Without this, Backspace behind the scrim would quietly delete
      // the thought the user is in the middle of saving.
      if (lib.sheet !== 'none') return;
      ...
      if (mod && e.key.toLowerCase() === 's') {
        e.preventDefault();
        // The map is already on disk; the reflex belongs to the account now.
        if (lib.enabled) lib.save();
        else app.toast('Saved on this device as you think');
        return;
      }
