...
import type { LibraryApi } from '../library/library';
import type { SessionApi } from '../account/session';
import type { Accent, Conviction, ThoughtType } from '@field/core';
import { ACCENTS, ACCENT_LABEL, CONVICTIONS, THOUGHT_TYPES, demoBoard, emptyBoard, subtree } from '@field/core';
import { exportJSON, exportPNG, exportSVG, pickJSONFile } from '../persist/io';
import { visibleWorldRect } from '@field/core';
...
export function buildCommands({ app, cam, lib, session, close }: CommandContext): Command[] {
  ...
  // Saving to an account, when this build has one to save to.
  if (lib.enabled) {
    const signedIn = session.status === 'signed-in';
    out.push({
      id: 'save',
      title: lib.remoteId && !lib.dirty ? 'Saved to your account' : 'Save map to my account',
      hint: signedIn ? undefined : 'creates an account',
      run: withClose(() => lib.save()),
    });
    if (signedIn) {
      out.push({
        id: 'save-as',
        title: 'Save as a new map',
        run: withClose(() => lib.openSheet('name')),
      });
      out.push({
        id: 'my-maps',
        title: 'Open a saved map',
        hint: `${lib.maps.length || ''}`.trim() || undefined,
        run: withClose(() => {
        ...
        children: () => [
          {
            ...
            run: withClose(() => {
              ...
              app.toast('Signed out. The map stays in this browser.');
            }),
          },
          { id: 'sign-out:no', title: 'Stay signed in', run: withClose(() => undefined) },
        ],
      });
    } else {
      out.push({
        id: 'sign-in',
        title: 'Sign in',
        run: withClose(() => lib.openSheet('account')),
      });
    }
  }
